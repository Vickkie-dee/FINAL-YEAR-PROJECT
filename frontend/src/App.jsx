  ...
  return (
    <div className="min-h-screen bg-white">
      <nav className="flex gap-4 p-4 border-b bg-gray-50">
        ...
        <button onClick={() => setView('validate')} className={`px-3 py-1 rounded ${view === 'validate' ? 'bg-blue-600 text-white' : 'text-gray-600'}`}>Validate</button>
      </nav>
      ...
      {view === 'validate' && <ValidationRunner />}
    </div>
  );
}
